...
export default async function handler(req, res) {
  ...
  const { denialCode, denialCategory, denialDescription, billedStr, deniedStr, additionalContext, rarcCode, rarcDescription, icd10Codes, payerName, placeOfService, modifierDetail, deniedLines } = req.body;
...
9. For Coverage Denials (CO-96, CO-5, CO-182, CO-204, CO-233, CO-250): When a payer name and ICD-10 codes are provided, you MUST search for the payer's Clinical Policy Bulletin (CPB) for the denied CPT code. Many CO-96 denials are misclassified medical necessity denials — the service IS covered when medically necessary with the right documentation. If the CPB confirms coverage when medically necessary AND the ICD-10 supports medical necessity, return LIKELY_FALSE. If it is a confirmed hard plan exclusion with no coverage pathway, return LIKELY_TRUE. If no CPB is found for smaller or regional payers, return CANNOT_DETERMINE and instruct the biller to request the CPB directly. Do NOT fabricate CPB numbers, policy titles, or effective dates.`;
  ...
  if (coverageRelevant) {
    ...
    const globalIcds = String(icd10Codes || "").split(/[,;\s]+/).map(s => s.trim()).filter(Boolean);
    // One entry per denied line; prefer the structured per-line data (each line's own dx).
    const lines = Array.isArray(deniedLines) && deniedLines.length
      ? deniedLines.map(l => ({ cpt: firstCpt(l.cpt), dx: l.dx ? firstIcd(l.dx) : null }))
      : [{ cpt: firstCpt(deniedStr), dx: null }];

    const lineResults = [];
    for (const ln of lines) {
      if (!ln.cpt) continue;
      // dx for this line: its own attached dx; else the global dx only if unambiguous (exactly one)
      let dx = ln.dx, dxSource = ln.dx ? "line" : null;
      if (!dx && globalIcds.length === 1) { dx = firstIcd(globalIcds[0]); dxSource = "global"; }
      else if (!dx && globalIcds.length > 1) { dxSource = "ambiguous"; }
      const rows = (jur.state && dx) ? await coverageLookup(jur.state, ln.cpt, dx) : null;
      lineResults.push({ cpt: ln.cpt, icd: dx, dxSource, matches: rows || [] });
    }

    if (lineResults.some(r => r.matches.length) || jur.isMedicareAdvantage) {
      coverageCheck = {
        state: jur.state,
      ...
      const blockLines = lineResults.map(r => {
        if (r.dxSource === "ambiguous")
          return `• CPT ${r.cpt}: diagnosis pointer AMBIGUOUS (multiple dx on the claim, none attached to this line) — cannot authoritatively pair; assess clinically and tell the biller to specify which dx was pointed to this line.`;
        if (!r.matches.length)
          return `• CPT ${r.cpt} + ICD ${r.icd || "?"}: no governing article found in the synced CMS data for this jurisdiction — fall back to clinical/grounded reasoning for this line.`;
        return r.matches.map(m => {
          const base = m.icd_status === "COVERED" ? `ICD ${r.icd} is COVERED (supports medical necessity)`
            : m.icd_status === "NONCOVERED" ? `ICD ${r.icd} is NON-covered (does NOT support)`
              : `ICD ${r.icd} is NOT linked to this CPT`;
          const conf = m.match_quality === "GROUP" ? " [CONFIRMED same group — authoritative]"
            : m.match_quality === "DIFFERENT_GROUP" ? " [ICD is in a DIFFERENT group than this CPT — does NOT establish coverage]"
              : " [co-listed only, unconfirmed — verify]";
          return `• CPT ${r.cpt} → ${m.doc_type} ${m.display_id}: ${base}.${conf}`;
        }).join("\n");
      }).join("\n");
      const docReqs = lineResults.flatMap(r => r.matches).find(m => m.doc_reqs)?.doc_reqs;
      coverageBlock = `AUTHORITATIVE CMS COVERAGE DATA — PER DENIED LINE (use as ground truth; do NOT contradict a CONFIRMED group match). Each denied CPT may have a DIFFERENT diagnosis and a DIFFERENT outcome — give a SEPARATE verdict for EACH line below and do NOT blend them.
Jurisdiction from payer "${payerName}": ${jur.state || "unresolved"}.
${blockLines}
${docReqs ? `Documentation requirements (from the LCD): ${docReqs}` : ""}
How to use: For each line, a CONFIRMED same-group COVERED → that line's denial is likely FALSE (appeal). A CONFIRMED NON-covered or DIFFERENT-group ICD → that line's denial is likely VALID (correct the dx or ABN). If lines differ, say so explicitly and break the verdict down line by line in the summary.${jur.isMedicareAdvantage ? "\nNOTE: payer appears to be Medicare ADVANTAGE — treat LCD/NCD as strong reference, not binding." : ""}

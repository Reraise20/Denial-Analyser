...
// Verify an AI-cited LCD/Article number against the authoritative coverage_documents
// table. Returns the real row if it exists, {notFound:true} if the query ran but the
// ID isn't in CMS data (likely fabricated), or null if we can't check (no config / NCD / error).
async function verifyCmsDocument(displayId) {
  const url = process.env.SUPABASE_URL, key = process.env.SUPABASE_SERVICE_ROLE_KEY || process.env.SUPABASE_ANON_KEY;
  if (!url || !key || !displayId) return null;
  const id = String(displayId).toUpperCase().replace(/\s+/g, "");
  if (!/^[AL]\d+$/.test(id)) return null;  // only L####/A#### style; NCD numbers (###.#) aren't in this table
  try {
    const res = await fetch(`${url}/rest/v1/coverage_documents?display_id=eq.${encodeURIComponent(id)}&select=display_id,title,url,status&limit=1`, {
      headers: { apikey: key, Authorization: `Bearer ${key}` },
    });
    if (!res.ok) return null;
    const rows = await res.json();
    return Array.isArray(rows) && rows.length ? rows[0] : { notFound: true };
  } catch { return null; }
}
...
10. RARC DEFINITION INTEGRITY — STRICTEST RULE — READ CAREFULLY:
    - If a RARC code was provided AND its official CMS/X12 description was supplied to you in the user prompt under "RARC Description": you MUST use that exact description verbatim as the rarcDefinition. Do not paraphrase, do not extend, do not "interpret."
    - If a RARC code was provided but NO description was supplied AND Google Search grounding is active in this session: you MUST search authoritative sources (x12.org/codes/remittance-advice-remark-codes, cms.gov, washington publishing wpc-edi.com) and use ONLY the exact text returned. Mark the rarcDefinition with the suffix " [verified via X12/CMS]".
    - If a RARC code was provided, NO description was supplied, AND grounding did not return an authoritative result: set rarcDefinition to "RARC meaning could not be verified — manual lookup required at x12.org/codes/remittance-advice-remark-codes" AND set verdict to CANNOT_DETERMINE. Do NOT invent a meaning. Do NOT pattern-match from the CARC. Plausible-sounding guesses about RARC meaning are the #1 source of incorrect verdicts in this tool.
    - NEVER state a RARC meaning that contradicts or modifies the official X12 definition. If you are uncertain whether your knowledge of a RARC is current, treat it as unverified and follow the rule above.
11. CARC DEFINITION INTEGRITY:
    - If the CARC description was supplied in the prompt: use it verbatim. Do not paraphrase.
    - If the CARC was not described and grounding is active: search and use the exact X12 definition.
    - If neither: set denialCodeDefinition to null and lower verdict confidence by 20 points.
12. VERDICT GUARDRAIL FOR UNVERIFIED CODES:
    - If either the CARC or the RARC meaning is unverified (no description supplied AND grounding did not return authoritative results), you MUST default to CANNOT_DETERMINE with confidence 0. The verdict downstream of a wrong code meaning is worthless. Better to tell the biller "I couldn't verify the codes — please look them up" than to produce a confident wrong answer.`;
  ...
  // both are modifier-specific RARCs that often pair with CO-16 on drug claims (JW/JZ for J-codes).
  const MODIFIER_RARCS = ["N56","N519","M78","M51","N657","M50","N822","N823"];
...
{"verdict":"TRUE|FALSE|LIKELY_TRUE|LIKELY_FALSE|CANNOT_DETERMINE","confidence":0-85 (max 85 if TRUE or FALSE; max 75 if LIKELY_TRUE or LIKELY_FALSE; must be 0 if CANNOT_DETERMINE),"verdictSummary":"one clear sentence explaining the verdict or why it cannot be determined","denialRuleExplained":"plain english explanation of ${denialCode}${rarcCode ? ` with remark code ${rarcCode}` : ""}","whyTrueDenial":[],"whyFalseDenial":[],"billingErrors":[],"correctiveActions":[],"appealSteps":[],"modifierGuidance":"null, OR a single advice string, OR (preferred when modifiers are present) an array of per-modifier strings each formatted '<MODIFIER> — valid/questionable: <reason>', with a final entry for any combination or sequence issue","documentationNeeded":[],"preventionTips":[],"timingAdvice":"deadline note or uncertainty disclaimer per rules","escalationPath":"general escalation process without fabricated contacts","lcdNcdReference":{"type":"LCD|NCD|NONE","articleNumber":"L##### or ###.# or null","title":"full policy title or null","url":"direct CMS URL to article or null","summary":"one sentence: does ICD-10 support coverage for this CPT under this policy, and what documentation is required"},"ncciSourceCycle":"the NCCI quarterly cycle or effective date STATED on the NCCI source you cited (e.g. 'Q1 2026' or 'effective January 1, 2026'), or null if no source states one — never guess","rarcDefinition":"STRICT RULES (Anti-Hallucination Rule 10): If a RARC description was supplied in the user prompt, output that exact text verbatim. If not supplied AND grounding returned an authoritative source (x12.org, cms.gov, wpc-edi.com), output the exact text from the source with suffix ' [verified via X12/CMS]'. If neither, output exactly 'RARC ${rarcCode || "XXX"} meaning could not be verified — manual lookup required at x12.org/codes/remittance-advice-remark-codes' and set verdict to CANNOT_DETERMINE. Null only if no RARC was provided.","denialCodeDefinition":"STRICT RULES (Anti-Hallucination Rule 11): If the CARC was described in the input, leave null. If the CARC was NOT described and grounding returned an authoritative source, output the exact X12 definition. If neither, output 'CARC ${denialCode} meaning could not be verified — manual lookup required at x12.org/codes/claim-adjustment-reason-codes' and lower confidence by 20."}`;
  ...
  // - ANY denial where the RARC was provided but is unknown to the local dictionary — this is the
  //   critical anti-hallucination fix. Without grounding, the model invents RARC meanings that
  //   plausibly fit the context but are factually wrong (e.g., guessing CO-16 + unknown RARC = "missing
  //   diagnosis" when the real meaning could be "missing modifier"). Forcing grounding routes the
  //   lookup to authoritative X12/CMS sources.
  // - ANY denial where the CARC was not described by the front-end dictionary (custom/uncommon code).
  // NOTE: grounding is incompatible with response_mime_type JSON mode — extract JSON from text response
  const rarcUnverified = !!rarcCode && !rarcDescription;
  const carcUnverified = !!denialCode && !denialDescription;
  const useGrounding =
  ...
  try {
    ...
    // ── ANTI-HALLUCINATION GUARD: unverified RARC + grounding fallback ──
    // If grounding was triggered specifically because the RARC was unknown to the local
    // dictionary, but grounding failed and fell back to no-search mode, the model has no
    // authoritative source for the RARC meaning. Returning a verdict here would be
    // hallucination (this is exactly what produced "N822 = missing diagnosis" before).
    // Return CANNOT_DETERMINE with a clear instruction to look up the code manually.
    if (useGrounding && !usedGrounding && rarcUnverified && !isAuth && !isCoverageDenial) {
      return res.status(200).json({
        verdict: "CANNOT_DETERMINE",
        confidence: 0,
        verdictSummary: `RARC ${rarcCode} is not in the local dictionary and live verification via X12/CMS was unavailable. A verdict based on guessed RARC meaning would be unreliable. Look up ${rarcCode} at x12.org/codes/remittance-advice-remark-codes before deciding next action.`,
        denialRuleExplained: `${denialCode}${denialDescription ? ` (${denialDescription})` : ""} was reported alongside remark code ${rarcCode}. Because ${rarcCode} could not be verified against an authoritative source in this session, the specific reason for the denial cannot be stated.`,
        whyTrueDenial: [],
        whyFalseDenial: [],
        billingErrors: [],
        correctiveActions: [
          `Manually verify the official meaning of RARC ${rarcCode} at https://x12.org/codes/remittance-advice-remark-codes`,
          `Cross-check with your MAC or payer remittance code reference for ${rarcCode}`,
          "Once the RARC meaning is confirmed, re-run this analysis with the description filled in, or apply the correction directly.",
        ],
        appealSteps: [
          "Do not file an appeal until the RARC meaning is confirmed — appeals based on misunderstood codes typically fail.",
          "If the verified RARC indicates a billing error (missing modifier, missing info), submit a corrected claim instead of an appeal.",
          "If the verified RARC indicates a coverage or medical-necessity issue, follow your standard appeal workflow with supporting documentation.",
        ],
        modifierGuidance: null,
        documentationNeeded: [
          `Official X12/CMS definition of RARC ${rarcCode}`,
          "Original EOB/ERA showing the CARC + RARC combination",
          "Claim line detail (CPT, modifiers, units, charge, paid amount)",
        ],
        preventionTips: [
          "Maintain an up-to-date internal RARC reference — X12 updates the master list at least three times per year (Jan/Jun/Oct).",
          "When a new RARC appears, document its meaning in your team knowledge base before working the denial.",
        ],
        timingAdvice: "Verify the exact appeal deadline directly with the payer — do not rely on an estimate.",
        escalationPath: "Resolve the code-meaning lookup first; escalation path depends on the verified denial reason.",
        lcdNcdReference: { type: "NONE", articleNumber: null, title: null, url: null, summary: null },
        rarcDefinition: `RARC ${rarcCode} meaning could not be verified — manual lookup required at x12.org/codes/remittance-advice-remark-codes`,
        wasGrounded: false,
        sources: null,
      });
    }
    ...
    // Verify any LCD/Article number the model cited against the authoritative table,
    // so fabricated "governing policy" numbers get flagged instead of trusted.
    let lcdVerification = null;
    const citedLcd = parsed?.lcdNcdReference?.articleNumber;
    if (citedLcd && parsed?.lcdNcdReference?.type && parsed.lcdNcdReference.type !== "NONE") {
      const v = await verifyCmsDocument(citedLcd);
      if (v && v.notFound) lcdVerification = { cited: citedLcd, status: "NOT_FOUND" };
      else if (v)          lcdVerification = { cited: citedLcd, status: "VERIFIED", title: v.title, url: v.url, docStatus: v.status };
    }

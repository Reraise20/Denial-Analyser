    ...
    try {
      const response = await fetch("/api/analyze", {
        ...
        body: JSON.stringify({
          ...
          modifierDetail,
          deniedLines: deniedLines.map(l => ({ cpt: l.cpt, modifier: l.modifier || null, dx: l.dx || null })),
        }),
      });
      ...
      <div style={{ minHeight:"100vh", background:DARK, fontFamily:"'Segoe UI',system-ui,sans-serif", padding:"20px 14px 48px" }}>
        <div style={{ maxWidth:800, margin:"0 auto" }}>
          ...
          {loading && (() => {
            ...
            const isCM    = ["CO-4"].includes(dc) || ["N56","N519","M78","M51","N657","M50"].includes((rarcCode||"").trim().toUpperCase());
          ...
          {result && !loading && ai && (
            <div ref={resultRef}>
                ...
                <div style={{ display:"flex", alignItems:"center", gap:14 }}>
                  ...
                  <div style={{ flex:1 }}>
                    ...
                    <div style={{ marginTop:8 }}>
                      {(ai.coverageCheck?.lines || []).some(l => (l.matches || []).some(m => m.match_quality === "GROUP"))
              ...
              <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, padding:"18px 20px" }}>
                ...
                {ai.coverageCheck && (ai.coverageCheck.lines?.some(l => l.matches?.length) || ai.coverageCheck.isMedicareAdvantage) && (
                  <div style={{ background:"rgba(52,211,153,0.05)", border:"1.5px solid rgba(52,211,153,0.3)", borderRadius:11, padding:"12px 15px", marginBottom:11 }}>
                    <div style={{ display:"flex", alignItems:"center", gap:8, marginBottom:8 }}>
                      ...
                      <span style={{ fontSize:11, fontWeight:700, color:"#34d399", textTransform:"uppercase", letterSpacing:"0.08em" }}>
                        CMS Coverage Check{ai.coverageCheck.state ? ` — ${ai.coverageCheck.state}` : ""}{(ai.coverageCheck.lines?.length > 1) ? " · per denied line" : ""}
                      </span>
                    </div>
                    ...
                    {(ai.coverageCheck.lines || []).map((ln, li, arr) => (
                      <div key={li} style={{ marginBottom:9, paddingBottom:9, borderBottom: li < arr.length-1 ? "1px solid rgba(148,163,184,0.18)" : "none" }}>
                        <div style={{ fontSize:12, fontWeight:700, color:"#c8d8ea", marginBottom:4 }}>
                          CPT {ln.cpt}{ln.icd ? <span style={{ color:"#7a9bbf", fontWeight:600 }}> → ICD {ln.icd}</span> : null}
                        </div>
                        {ln.dxSource === "ambiguous"
                          ? <p style={{ margin:0, fontSize:11, color:"#f59e0b", lineHeight:1.6 }}>⚠️ Diagnosis pointer ambiguous — multiple diagnoses on the claim, none attached to this line. Enter this line as “CPT DX” (e.g. “45378 K63.5”) for an authoritative result.</p>
                          : !ln.matches?.length
                            ? <p style={{ margin:0, fontSize:11, color:"#7a9bbf", lineHeight:1.6, fontStyle:"italic" }}>No governing CMS article found for this line in the synced data — its verdict is AI-reasoned, not authoritative.</p>
                            : ln.matches.map((m, mi) => {
                                const q = m.match_quality;
                                let st;
                                if (m.icd_status === "COVERED")         st = { c:"#34d399", label:"COVERED",         t:`ICD ${ln.icd} supports medical necessity for CPT ${ln.cpt}` };
                                else if (m.icd_status === "NONCOVERED") st = { c:"#f87171", label:"NON-COVERED",     t:`ICD ${ln.icd} does NOT support medical necessity for CPT ${ln.cpt}` };
                                else if (q === "DIFFERENT_GROUP")       st = { c:"#f59e0b", label:"DIFFERENT GROUP", t:`ICD ${ln.icd} is in this article but under a different procedure group — it does not establish coverage for CPT ${ln.cpt}` };
                                else                                    st = { c:"#94a3b8", label:"NOT LINKED",      t:`ICD ${ln.icd} is not linked to CPT ${ln.cpt} in this policy` };
                                ...
                                return (
                                  <div key={mi} style={{ marginTop: mi > 0 ? 6 : 0 }}>
                                    ...
                                    {m.title && <p style={{ margin:0, fontSize:11.5, color:"#7a9bbf", lineHeight:1.6 }}>{m.title}</p>}
                ...
                <div style={{ background:"rgba(56,189,248,0.04)", border:`1.5px solid rgba(56,189,248,0.15)`, borderRadius:11, padding:"13px 15px", marginBottom:11 }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {(result.codeInfo?.category === "NCCI Bundling" || result.denialCode === "CO-59" || result.denialCode === "CO-97" || result.denialCode === "CO-78" || result.denialCode === "CO-231" || result.denialCode === "CO-4" || ["N56","N519","M78","M51","N657","M50"].includes(result.rarcCode)) && (

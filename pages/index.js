...
const DENIAL_CODES = {
  ...
  // ── NCCI BUNDLING ─────────────────────────────────────────────────────────
  "CO-59":  { label:"Multiple Procedures",     category:"Multiple Procedures", color:"#f97316", icon:"🔗", description:"Payment adjusted — multiple/concurrent procedure reduction rule applied" },
...
const RARC_CODES = {
  // ── MISSING / INCOMPLETE INFO (M-series) ─────────────────────────────────
  "M76":  { description: "Missing/incomplete/invalid diagnosis or condition code" },
  "M77":  { description: "Missing/incomplete/invalid place of service" },
  "M79":  { description: "Missing/incomplete/invalid charge — resubmit with corrected amount" },
  "M80":  { description: "Not covered when performed in the same session as a previously processed service" },
  "M86":  { description: "Service denied because payment already made for same/similar procedure on same date" },
  "M115": { description: "This item is not covered when the patient is under a specified age" },
  "M119": { description: "Missing/incomplete/invalid date" },
  "M124": { description: "Missing/incomplete/invalid indications for this service" },
  "M127": { description: "Missing/incomplete/invalid prior authorization or referral" },
  "M131": { description: "Physician already reimbursed for services provided in this setting" },
  "M133": { description: "Claim not covered by this payer/contractor — redirect or coordinate benefits" },
  "M144": { description: "Pre-authorization/notification not required for this service" },
  // ── MA-series (Medicare Administrative) ──────────────────────────────────
  "MA01": { description: "If you disagree with the approved amount, you may appeal — submit documentation" },
  "MA04": { description: "Secondary payment cannot be considered without primary payer identity and payment info" },
  "MA13": { description: "You may be subject to penalties if you bill the patient for amounts not reported with PR group code" },
  "MA18": { description: "Claim information is also being forwarded to the patient's supplemental insurer" },
  "MA37": { description: "Provider's group membership termination date is prior to the date of service" },
  "MA61": { description: "Missing/incomplete/invalid social security number or health insurance claim number" },
  "MA62": { description: "Alert: this is a duplicate claim/service" },
  "MA63": { description: "Missing/incomplete/invalid patient identifier" },
  // ── N-series (General Remark) ─────────────────────────────────────────────
  "N4":   { description: "Missing/incomplete/invalid prior authorization or referral number" },
  "N19":  { description: "Procedure code is incidental to the primary procedure — not separately payable" },
  "N20":  { description: "Service not separately payable with another service rendered on the same date" },
  "N56":  { description: "Procedure code billed is not correct/valid for the services billed or date of service" },
  "N95":  { description: "This provider type or specialty may not bill this service" },
  "N104": { description: "This claim/service is not payable under the payer's claims jurisdiction area" },
  "N115": { description: "This decision was based on a Local Coverage Determination (LCD)" },
  "N130": { description: "Consult your contractual agreement for restrictions and billing details" },
  "N180": { description: "This item or service does not meet the criteria for the category under which it was billed" },
  "N362": { description: "Number of days or units of service exceeds the acceptable maximum" },
  "N381": { description: "Records indicate an Advanced Beneficiary Notice (ABN) was not on file when services were rendered" },
  "N519": { description: "Invalid combination of HCPCS modifiers — review modifier usage" },
  "N570": { description: "Missing/incomplete/incomplete credentialing data for the rendering provider" },
  "N706": { description: "Incomplete/invalid treatment plan" },
  "N522": { description: "Duplicate of a claim processed, or to be processed, as a crossover claim" },
  "N645": { description: "This service is not payable under the current fee schedule — check applicable payment policy" },
  "N657": { description: "Coverage for this service has been excluded under this plan" },
};
...
function Section({ icon, title, items, color, note, payerName }) {
    ...
    <div style={{ background:`${color}0d`, border:`1.5px solid ${color}30`, borderRadius:12, padding:"14px 16px", marginBottom:11 }}>
      ...
      {items.map((item, i) => {
        ...
        const lcdHref = cptMatches.length >= 1
          ? `https://www.cms.gov/medicare-coverage-database/search.aspx?keyword=${cptMatches[0]}&keywordType=code`
        ...
        // Auth link
        const mentionsAuth = /(prior\s*auth|authorization|auth\s*required|precert|pre-cert|clinical\s*policy)/i.test(item);
        const authCpt = cptMatches[0] || null;
        const authHref = payerName && authCpt
          ? `https://www.google.com/search?q=${encodeURIComponent(`${payerName} prior authorization requirements CPT ${authCpt}`)}`
          : payerName
          ? `https://www.google.com/search?q=${encodeURIComponent(`${payerName} prior authorization policy`)}`
          : authCpt
          ? `https://www.google.com/search?q=${encodeURIComponent(`prior authorization requirements CPT ${authCpt}`)}`
          : "https://www.google.com/search?q=prior+authorization+requirements+medical+billing";
        const authLabel = payerName && authCpt
          ? `🔐 Look up ${payerName} auth policy for CPT ${authCpt} ↗`
          : payerName
          ? `🔐 Look up ${payerName} prior auth policy ↗`
          : authCpt
          ? `🔐 Look up auth requirements for CPT ${authCpt} ↗`
          : "🔐 Look up Prior Auth Requirements ↗";
        ...
        return (
          <div key={i} style={{ marginBottom:8 }}>
            ...
            {(mentionsNcci || mentionsLcd || mentionsAuth) && (
              <div style={{ display:"flex", flexWrap:"wrap", gap:6, marginTop:5, marginLeft:21 }}>
                ...
                {mentionsAuth && (
                  <a href={authHref} target="_blank" rel="noopener noreferrer"
                    style={{ display:"inline-flex", alignItems:"center", gap:5, padding:"3px 10px", borderRadius:5, background:"rgba(56,189,248,0.1)", border:"1px solid rgba(56,189,248,0.3)", textDecoration:"none", fontSize:11, fontWeight:700, color:ACCENT }}>
                    {authLabel}
                  </a>
                )}
              </div>
...
export default function DenialAnalyzer() {
  ...
  function reset() {
    ...
    setRarcCode(""); setIcd10Codes(""); setPayerName("");
  }
          ...
          <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, padding:"20px", marginBottom:12 }}>
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#3a5a7a", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:6, display:"flex", alignItems:"center", gap:7 }}>
                ...
                Payer Name
                <span style={{ fontWeight:400, color:"#2a4a6a", textTransform:"none", fontSize:11 }}>(recommended — required for auth denial grounding)</span>
              </div>
              <input
                value={payerName}
              ...
              <div style={{ display:"flex", gap:8, alignItems:"flex-start" }}>
                ...
                {rarcCode && !RARC_CODES[rarcCode.trim().toUpperCase()] && (
                  <div style={{ flex:1, padding:"8px 12px", borderRadius:8, background:"rgba(100,116,139,0.08)", border:"1.5px solid rgba(100,116,139,0.2)", fontSize:12, color:"#64748b", lineHeight:1.5 }}>
                    Code not in local dictionary — will be sent to AI as entered
                  </div>
                )}
              </div>
            </div>
            ...
            <div style={{ marginBottom:14 }}>
              <div style={{ fontSize:11, fontWeight:700, color:"#3a5a7a", textTransform:"uppercase", letterSpacing:"0.1em", marginBottom:6, display:"flex", alignItems:"center", gap:7 }}>
                ...
                <span style={{ fontWeight:400, color:"#2a4a6a", textTransform:"none", fontSize:11 }}>(recommended for medical necessity denials)</span>
              </div>
              <input
                value={icd10Codes}
          ...
          {result && !loading && ai && (
            <div ref={resultRef}>
              ...
              <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, padding:"18px 20px" }}>
                ...
                {isTrue  && <Section icon="❌" title="Why This Denial Is Valid"          color="#ef4444" items={ai.whyTrueDenial} payerName={payerName} />}
                {isFalse && <Section icon="✅" title="Why This Denial Is Incorrect"      color="#22c55e" items={ai.whyFalseDenial} payerName={payerName} />}
                {isFalse && ai.whyTrueDenial?.length>0  && <Section icon="⚠️" title="What Payer May Cite"        color="#f59e0b" items={ai.whyTrueDenial} payerName={payerName} />}
                {isTrue  && ai.whyFalseDenial?.length>0 && <Section icon="💡" title="Potentially Disputable"     color="#a78bfa" items={ai.whyFalseDenial} payerName={payerName} />}
                {isUncertain && ai.whyTrueDenial?.length>0  && <Section icon="⚖️" title="Factors Supporting the Denial"   color="#f59e0b" items={ai.whyTrueDenial} note="Cannot confirm without payer-specific contract terms or clinical documentation." payerName={payerName} />}
                {isUncertain && ai.whyFalseDenial?.length>0 && <Section icon="💡" title="Factors Supporting a Dispute"    color="#94a3b8" items={ai.whyFalseDenial} note="Review against your provider agreement and the actual remittance before acting." payerName={payerName} />}
                {isUncertain && (
                ...
                {ai.billingErrors?.length>0    && <Section icon="🔴" title="Billing Errors Identified"           color="#f87171" items={ai.billingErrors} note="These are the likely root cause of the denial." payerName={payerName} />}
                {ai.modifierGuidance           && <Section icon="🏷️" title="Modifier Guidance"                  color="#c084fc" items={[ai.modifierGuidance]} payerName={payerName} />}
                {isTrue  && ai.correctiveActions?.length>0 && <Section icon="🔧" title="How to Correct & Resubmit"  color={ACCENT}   items={ai.correctiveActions} payerName={payerName} />}
                {isFalse && ai.appealSteps?.length>0        && <Section icon="⚖️" title="Steps to Appeal & Get Paid" color="#22c55e"  items={ai.appealSteps} payerName={payerName} />}
                {isTrue  && ai.appealSteps?.length>0        && <Section icon="⚖️" title="Appeal Steps (if disputing)" color="#a78bfa" items={ai.appealSteps} payerName={payerName} />}
                ...
                {ai.preventionTips?.length>0 && <Section icon="🛡️" title="Prevention Tips" color="#34d399" items={ai.preventionTips} payerName={payerName} />}

                {/* Grounding Sources */}
                {(ai.sources?.length > 0 || ai.usedQueries?.length > 0) && (
                  <div style={{ background:"rgba(56,189,248,0.04)", border:"1.5px solid rgba(56,189,248,0.2)", borderRadius:11, padding:"13px 15px", marginBottom:11 }}>
                    <div style={{ fontSize:11, fontWeight:700, color:ACCENT, textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:10 }}>
                      🌐 Sources Referenced by AI — Verify Manually
                    </div>
                    {ai.usedQueries?.length > 0 && (
                      <div style={{ marginBottom:9 }}>
                        <div style={{ fontSize:10, color:"#3a5a7a", fontWeight:600, marginBottom:5, textTransform:"uppercase", letterSpacing:"0.06em" }}>🔍 Searches AI performed:</div>
                        <div style={{ display:"flex", flexWrap:"wrap", gap:5 }}>
                          {ai.usedQueries.map((q, i) => (
                            <a key={i} href={`https://www.google.com/search?q=${encodeURIComponent(q)}`} target="_blank" rel="noopener noreferrer"
                              style={{ fontSize:10, color:ACCENT, background:"rgba(56,189,248,0.08)", border:"1px solid rgba(56,189,248,0.25)", borderRadius:5, padding:"3px 8px", textDecoration:"none", fontWeight:600 }}>
                              {q} ↗
                            </a>
                          ))}
                        </div>
                      ...
                      <div style={{ display:"flex", flexDirection:"column", gap:7 }}>
                        {ai.sources.map((s, i) => (
                          <a key={i} href={s.url} target="_blank" rel="noopener noreferrer"
                            style={{ display:"flex", alignItems:"center", gap:9, padding:"8px 11px", borderRadius:8, background:"rgba(14,116,144,0.08)", border:"1px solid rgba(14,116,144,0.25)", textDecoration:"none" }}>
                ...
                <div style={{ background:"rgba(56,189,248,0.04)", border:`1.5px solid rgba(56,189,248,0.15)`, borderRadius:11, padding:"13px 15px", marginBottom:11 }}>
                  ...
                  <div style={{ display:"flex", flexDirection:"column", gap:8 }}>
                    {(result.codeInfo?.category === "NCCI Bundling" || result.denialCode === "CO-59" || result.denialCode === "CO-97" || result.denialCode === "CO-78" || result.denialCode === "CO-231") && (
                    ...
                    {(result.codeInfo?.category === "Medical Necessity") && (
                      <a href="https://www.cms.gov/medicare-coverage-database/search.aspx" target="_blank" rel="noopener noreferrer"
                    ...
                    <a href="https://www.cms.gov/medicare-coverage-database/search.aspx" target="_blank" rel="noopener noreferrer"
                      ...
                      <div>
                        ...
                        <div style={{ fontSize:11, color:"#6a7a8a" }}>Search all LCD, NCD, and Article policies</div>
                      </div>
                    </a>

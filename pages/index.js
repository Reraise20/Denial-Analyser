          ...
          {result && !loading && ai && (
            <div ref={resultRef}>
              ...
              <div style={{ background:CARD, border:`1px solid ${BORDER}`, borderRadius:16, padding:"18px 20px" }}>
                ...
                {ai.coverageCheck && (ai.coverageCheck.matches?.length > 0 || ai.coverageCheck.isMedicareAdvantage) && (
                  <div style={{ background:"rgba(52,211,153,0.05)", border:"1.5px solid rgba(52,211,153,0.3)", borderRadius:11, padding:"12px 15px", marginBottom:11 }}>
                    ...
                    {(ai.coverageCheck.matches || []).map((m, i, arr) => {
                      const st = m.icd_status === "COVERED"
                        ? { c:"#34d399", t:`ICD-10 ${ai.coverageCheck.icd10} supports medical necessity (on the covered list)` }
                        : m.icd_status === "NONCOVERED"
                          ? { c:"#f87171", t:`ICD-10 ${ai.coverageCheck.icd10} does NOT support medical necessity (non-covered list)` }
                          : { c:"#f59e0b", t:`ICD-10 ${ai.coverageCheck.icd10} is not listed in this policy's ICD-10 tables` };
                      return (
                        <div key={i} style={{ marginBottom:8, paddingBottom:8, borderBottom: i < arr.length-1 ? "1px solid rgba(148,163,184,0.12)" : "none" }}>
                          <div style={{ display:"flex", alignItems:"center", gap:7, flexWrap:"wrap", marginBottom:3 }}>
                            <span style={{ fontSize:10, fontWeight:700, color:st.c, background:`${st.c}18`, border:`1px solid ${st.c}55`, borderRadius:4, padding:"2px 7px" }}>{m.icd_status}</span>
                            {m.url
                ...
                {(Array.isArray(ai.modifierGuidance) ? ai.modifierGuidance.length>0 : !!ai.modifierGuidance) && <Section icon="🏷️" title="Modifier Analysis"                  color="#c084fc" items={Array.isArray(ai.modifierGuidance) ? ai.modifierGuidance : [ai.modifierGuidance]} />}

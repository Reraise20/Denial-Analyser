    ...
    if (rows || jur.isMedicareAdvantage) {
      ...
      if (rows) {
        const lines = rows.map(r =>
          `• ${r.doc_type} ${r.display_id} "${r.title}": ICD-10 ${icd} is ${r.icd_status === "COVERED"
            ? "ON the covered list (supports medical necessity)"
            : r.icd_status === "NONCOVERED"
              ? "ON the NON-covered list (does NOT support medical necessity)"
              : "NOT listed in this policy's ICD-10 tables"} for CPT ${cpt}.`).join("\n");
        coverageBlock = `AUTHORITATIVE CMS COVERAGE DATA (use as ground truth — do NOT contradict this with web/memory):
Jurisdiction resolved from payer "${payerName}": ${jur.state}. The following come from the synced CMS Medicare Coverage Database for this jurisdiction:
${lines}
${rows.find(r => r.doc_reqs) ? `Documentation requirements (from the LCD): ${rows.find(r => r.doc_reqs).doc_reqs}` : ""}
Base your verdict on this. If the ICD-10 is on the covered list, a medical-necessity denial is likely FALSE (payable) — guide toward appeal with the documentation above. If NON-covered or NOT listed, the denial is likely valid — guide toward correcting the diagnosis or an ABN/patient-responsibility path.${jur.isMedicareAdvantage ? "\nNOTE: payer appears to be a Medicare ADVANTAGE plan — treat the LCD/NCD as a strong reference, NOT a binding rule, since MA plans may set their own policy." : ""}
